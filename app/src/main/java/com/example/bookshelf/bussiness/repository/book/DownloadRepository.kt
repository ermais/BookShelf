...
class DownloadRepository(private val db: BookDatabase) {
    ...
    suspend fun deleteDownloadBook(_download: DownloadEntity) =
        db.downloadDao().deleteDownload(_download)

    suspend fun updateDownloadBook(_download: DownloadEntity) =
        db.downloadDao().updateDownload(_download)
